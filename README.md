...
- **Rotation.** `revvault rotate <provider>` runs a full rotation against `local`, `http`, `neon`, or `ed25519-keypair` providers (auto-selected via `type` in `rotation.toml`): reads the current key, dispatches the provider, validates the returned value's shape, writes the new key back, applies the optional post-rotation sync hook, runs `post_rotate` hooks (warn-on-failure) and an optional strict `verify` gate, then appends a log entry. `--dry-run` previews the plan without touching the vault or any API. `rotation-status` reports the last 10 log entries.
